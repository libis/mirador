var DownloadMenu = {
  ...
  imageUrlTemplate: Handlebars.compile(
    '{{imageBaseUrl}}/full/{{size}}/0/default.jpg'
  ),

  /* the template for the link menu */
  menuTemplate: Handlebars.compile([
 ...
 sizes: ['full','2400,','1200,','800,','300,'],
  ...
  extractImageUrls: function(viewerWindow){
    ...
    sizeNames = ['Current view', 'X-large','Large','Medium','Small'];

      imageUrls.push({
        'href': viewerWindow.currentImageMode !== 'ImageView' ? '#' : this.imageUrlTemplate({
          'imageBaseUrl': imageBaseUrl, 'size': size
        }),
        'title': size === 'full' ? sizeNames[index] + ' ' +currentImage.width + 'x' + currentImage.height : sizeNames[index] + ' ' + parseInt(size) + 'x' + Math.ceil(parseInt(size) * ratio)
      });
    }.bind(this));
    for (i=0;i<imageUrls.length;i++){
       imageUrls[i].href = imageUrls[i].href.replace(/&#x3D;/g, '=');
    }
    return imageUrls;
  },
