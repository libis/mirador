...
/*SPA */        clientLogo:    this.state.getStateProperty('mainMenuSettings').clientLogo
            }));
        ...
        '{{/if}}',
        '{{#if clientLogo}}',
          '<ul class="client-logo {{mainMenuCls}}">',
/*SPA*/     '{{clientlogo clientLogo}}',
          '</ul>',
        '{{/if}}',
    ...
    /* SPA */
        $.Handlebars.registerHelper('clientlogo', function (clientLogo) {
        return new $.Handlebars.SafeString(
            processUserBtn(clientLogo).get(0).outerHTML
        );
    });
